...
class Nim {
    ...
    public int eval(int player) {
         ...
         */
    	int suma = heaps[0] + heaps[1] + heaps[2];
    	if(suma == 1) {
    		return -Inf;
    	}
    	if(ended()) {
    	...
    	int score = (int) (((suma % 3 + suma % 2) / (double)suma) * 100);
    	if(score == 0) {
    		score += xuma;
    	}
    ...
    public boolean apply_move(Move move) {
         ...
         */
    	if(move.heap < 0 || move.heap >= 3 || move.amount > 3 || move.amount < 1)
    		return false;
...
class P1 {
    ...
    public static Pair<Integer, Move> minimax(Nim init, int player, int depth) {
        ...
        Pair<Integer, Move> scoreMove = new Pair<>(max, new Move());
        Move best = new Move();
        int score;
        Nim clona;
        
        for(Move move:moves) {
        	clona = (Nim) init.clone();
        	clona.apply_move(move);
        	scoreMove = minimax(init, -player, depth - 1);
        	score = - scoreMove.first;
	        if(score > max) {
	        	max = score;
	        	best = move;
	        }
