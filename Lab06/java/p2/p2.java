...
class Reversi {
    ...
    public int eval(int player) {
    	...
    	int suma_player = 0, suma_oponent = 0;
    	for(int i = 0; i < N; i++) {
    		for(int j = 0; j < N; j++) {
    			if(data[i][j] == 0) {
    				suma_oponent++;
    	...
    	if(player < 0) {
    		int aux = suma_player;
    		suma_player = suma_oponent;
    		suma_oponent = aux;
    	}
    	
        return (int) (((double)suma_player/suma_oponent) * 100);
    }
...
class P2 {
    ...
    static Pair<Integer, Move> minimax(Reversi init, int player, int depth) {
         ...
         */
    	Pair<Integer, Move> returnPair;
    	Move bestMove = new Move(player);
    	int max = -Reversi.Inf, score = 0;
    	
    	for(int i = 0; i < Reversi.N; i++) {
    		for(int j = 0; j < Reversi.N; j++) {
    			
    			Move next = new Move(player, i, j);
    			Reversi clona = (Reversi) init.clone();
    			
    			if(clona.apply_move(next)) {
    				returnPair = minimax(clona, -player, depth - 1);
    				score = -returnPair.first;
