...
show_help() {
  ...
  echo -e "executiei."
  echo ""
  echo -e "De asemenea, \"./check.sh pack <nume.zip>\" realizeaza o arhiva zip."
  echo -e "Exemplu: ./check.sh pack 321CA_GigelGigel_lab00.zip"
  echo ""
...
pack() {
  ...
  zip -FSr "${1}" "${skel_dir}/" &>/dev/null && echo "Done packing!"
  command popd &>/dev/null
}
